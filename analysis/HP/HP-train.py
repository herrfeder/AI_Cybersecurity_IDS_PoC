...
def best_opti_fitted_model(
        model_list, models_name_list, param_dict_list, x, y, xtest, ytest):
    xnew = x
    ynew = y
    xtest = xtest
    ytest = ytest
    ...
    for i in range(len(model_list)):
        ...
        random_search = RandomizedSearchCV(
            my_models[i], param_distributions=param_dict_list[i], n_iter=n_iter_search, cv=5, iid=False)
        start = time()
        ...
        bestparams, bestscore = get_best_params_and_report(
    ...
    return pd.DataFrame(data=d), [bestIndex,
                                  bestmodel, bestACC, bestPre, bestF1]
...
# ### Auswahlkriterium
# Die Selektion findet aufgrund der Bedingung statt, dass das Modell mit
# dem größten Messungen aus F1, Precision und Recall gewählt wird
...
dataframe_with_scores, bestmodel = best_opti_fitted_model(
    my_models, my_models_name, param_dist, xnew_train, ynew_train, xnew_test, ynew_test)
...
print(dataframe_with_scores.sort_values(
    by=['Precision', 'Fmeasure', 'Accuracy'], ascending=False).head(10))


print('##### THE 4 - BEST MODELS SORTED BY Precision, Fmeasure and Accuracy ARE -> ####\n')
print(dataframe_with_scores.sort_values(
    by=['Precision', 'Fmeasure', 'Accuracy'], ascending=False).iloc[0:4])


print('##### THE 10 - BEST MODELS SORTED BY Fmeasure ONLY ARE -> ####\n')
print(dataframe_with_scores.sort_values(
    by=['Fmeasure'], ascending=False).iloc[0:10])
...
for i in range(len(my_models)):
    rf_roc_auc_array.append(roc_auc_score(
        ynew_test, my_models[i].predict(xnew_test)))
    fpr, tpr, thresholds = roc_curve(
        ynew_test, my_models[i].predict_proba(xnew_test)[:, 1])
    fpr_array.append(fpr)
    tpr_array.append(tpr)
    thresholds_array.append(thresholds)
    plt.plot(fpr, tpr, label=my_models_name[i] +
             ' (area = %0.2f)' % rf_roc_auc_array[i])
